...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (quitFlag) {
            ...
            System.out.println("    Enter 7 to see your Client list");
            System.out.println("    Enter 8 to add new Clothes");
            ...
            switch (res) {
                ...
                case "7":
                new Clients().showClients();
                break;

                case "8":
                    int id6 = onlyCheckInt(reader, qId);
                    int qty6 = onlyCheckInt(reader, qQty);
                    System.out.println("Enter the price: ");
                    int euro6 = onlyCheckInt(reader, qEuro);
                    int cent6 = onlyCheckInt(reader, qCents);
                    stock.addClothes(id6, new Money(euro6, cent6), qty6, selectClothesSize(reader), "Manufaturer");
                    break;
    ...
    public static sizeClothes selectClothesSize(Scanner reader) {
        System.out.println("Select size");
        for (sizeClothes sizeClothes : sizeClothes.values()) {
            System.out.println(sizeClothes);
        }

        return switch(sizeClothes.valueOf(reader.next().toUpperCase())) {
            case L -> sizeClothes.L;
            case M -> sizeClothes.M;
            case S -> sizeClothes.S;
            case XL -> sizeClothes.XL;
            default -> {
                yield null;
            }
        };
